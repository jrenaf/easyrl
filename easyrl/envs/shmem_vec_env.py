...
from easyrl.utils.vec_env_util import dict_to_obs
...
class ShmemVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None, context='spawn'):
        ...
        self.obs_bufs = [
            {k: ctx.Array(_NP_TO_CT[self.obs_dtypes[k].type],
                          int(np.prod(self.obs_shapes[k]))) for k in self.obs_keys}
            for _ in env_fns
        ]
        self.parent_pipes = []
        self.procs = []
    ...
    def _decode_obses(self, obs):
        ...
        for k in self.obs_keys:
            ...
            o = [np.frombuffer(b.get_obj(),
                               dtype=self.obs_dtypes[k]).reshape(self.obs_shapes[k]) for b in bufs]
            result[k] = np.array(o)
